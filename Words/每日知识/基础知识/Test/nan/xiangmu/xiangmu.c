...
int manage_system()
{
    ...
    memset（stu_stu,0,sizeof (STU)*100);
    scanf("%d",&key);
    while(key!=0)
    {
        if (key==1)
        {
            //增加成绩
            printf（"请输入学生的名字"）；
            scanf（"%s",&stu_[num].name);
            printf("请输入学生语文成绩")；
            scanf("%d",&stu_[num].chinese);
            printf ("请输入学生数学成绩")；
            scanf（"%d",&stu_[num].math);
            printf ("请输入学生英语成绩")；
            scanf（"%d",&stu_[num].english);
            num++
            printf("输入成功！\n")
        }
        else if (key==2)
        ...
        for (i=num;i>=index;i++)
        {
            strucp(str_stu[index].name,str_stu[index+1].name)
            tr_stu[index].chinese= str_stu[index+1].chinese;
